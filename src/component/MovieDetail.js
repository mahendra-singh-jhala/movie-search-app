...
const MovieDetail = ({favourite, addToFavourite}) => {
    ...
    const isFavourite = favourite && movie.imdbID ? favourite.some(fav => fav.imdbID === movie.imdbID) : false;
    ...
    return (
        <div className="text-white flex p-4">
            ...
            <div className="mt-4 ms-4">
                ...
                <p className="text-lg mt-4">Add To Favourite : <button onClick={() => addToFavourite(movie)} className={`font-bold py-2 px-4 rounded ${isFavourite ? 'text-yellow-500' : 'text-white'}`}><i class="fa-solid fa-star"></i></button></p>
            </div>
        </div>
    )
}
